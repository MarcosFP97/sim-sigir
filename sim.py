...
def eval_serp(serp):
  scores = pd.read_csv('./data/overall_bias.csv', sep=';')
...
def simulate(corpus):
    ...
    for sample in ll[33:]: # samples: ##### THIS!!!
        ...
        ###### SALVAR EN UN CSV
        session.to_csv(f'./sim_output/{first_step_mode}_{second_step_mode}/session_{first_step_mode}_{second_step_mode}_steps_{sample}_{count}.csv', sep=',', index=False, header=True)
        print("======SESSION ENDED========")
        count+=1
...
if __name__=="__main__":
    ...
    corpus = pd.read_csv('corpus_def.csv')
    simulate(corpus)